// app/components/gestor/usuarios/TabelaUsuarios.tsx
"use client";
  ...
  const usuariosFiltrados = useMemo(() => {
    return usuarios.filter((u) => {
      ...
      const perfilMap: Record<string, UserRole> = {
        Aluno: "aluno", Colaborador: "colaborador", Administrador: "adm",
      };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-[#FFDE00] hover:bg-[#e6c800] text-[#252525] text-sm font-bold rounded-[4px] transition-colors shadow-sm"
        >
          <IconPlus size={18} />
          Novo usuário
        </button>
      ...
      {erroCarga && !loading && (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-slate-600 dark:text-slate-300 font-medium">Falha ao carregar usuários.</p>
        ...
        <div className="w-full flex flex-col bg-white dark:bg-[#1a1a1a] rounded-2xl shadow-sm border border-slate-100 dark:border-[#303030] overflow-hidden">
          ...
          <div className="flex items-start justify-between p-5 border-b border-slate-100 dark:border-[#2a2a2a] shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-slate-900 dark:text-white text-base leading-tight">Usuários</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{usuariosFiltrados.length} no total</p>
              </div>
            </div>
            <div className="hidden sm:flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400 mr-2">
              <span><strong className="text-emerald-600 dark:text-emerald-400">{totalAtivos}</strong> ativos</span>
              {totalExcluidos > 0 && (
                <span><strong className="text-red-500">{totalExcluidos}</strong> excluídos</span>
              )}
            </div>
          </div>
          ...
          <div className="overflow-auto flex-1">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 dark:bg-[#1e1e1e] sticky top-0 z-10">
                <tr>
                  <th className="px-4 py-3 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Usuário</th>
                  <th className="hidden sm:table-cell px-4 py-3 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">E-mail</th>
                  <th className="hidden md:table-cell px-4 py-3 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Matrícula</th>
                  <th className="hidden lg:table-cell px-4 py-3 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Perfil</th>
                  <th className="hidden lg:table-cell px-4 py-3 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Status</th>
                  <th className="px-4 py-3 text-right text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Ações</th>
              ...
              <tbody>
                ...
                {loading && Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="border-b border-slate-100 dark:border-[#2a2a2a] animate-pulse">
                    ...
                    <td className="hidden sm:table-cell px-4 py-3"><div className="h-3 bg-slate-200 dark:bg-[#2a2a2a] rounded w-40" /></td>
                    <td className="hidden md:table-cell px-4 py-3"><div className="h-3 bg-slate-200 dark:bg-[#2a2a2a] rounded w-24" /></td>
                    <td className="hidden lg:table-cell px-4 py-3"><div className="h-5 bg-slate-200 dark:bg-[#2a2a2a] rounded-full w-20" /></td>
                    <td className="hidden lg:table-cell px-4 py-3"><div className="h-5 bg-slate-200 dark:bg-[#2a2a2a] rounded-full w-16" /></td>
                    <td className="px-4 py-3" /><td className="lg:hidden px-3 py-3" />
                  </tr>
                ))}
                  ...
                  <tr>
                    <td colSpan={7} className="py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm text-slate-400 font-medium">Nenhum usuário encontrado com esses filtros.</p>
                      </div>
                    </td>
                  </tr>
                )}
      ...
      <ModalDetalhesUsuario usuario={modalDetalhes} onFechar={() => setModalDetalhes(null)} onEditar={(u) => setModalForm(u)} />
...
function LinhaUsuario({ usuario: u, ehOProprioAdmin, onDetalhes, onEditar, onExcluir, onRestaurar }: LinhaUsuarioProps) {
  ...
  return (
    <>
      ...
      >
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate max-w-[120px]">{u.nome}</p>
        ...
        {/* E-mail — sm+ */}
        <td className="hidden sm:table-cell px-4 py-3 text-base text-slate-600 dark:text-slate-300">{u.email}</td>

        {/* Matrícula — md+ */}
        <td className="hidden md:table-cell px-4 py-3 text-sm font-mono text-slate-500 dark:text-slate-400">{u.matricula}</td>
        ...
        <td className="hidden lg:table-cell px-4 py-3 text-sm">
          {excluido ? (
            <span className="inline-flex items-center gap-1 text-red-500 dark:text-red-400 font-semibold text-sm"><IconCircleX size={18} /> Excluído</span>
          ) : u.ativo ? (
            <span className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-semibold text-sm"><IconCircleCheck size={18} /> Ativo</span>
          ) : (
            <span className="inline-flex items-center gap-1 text-slate-400 dark:text-slate-500 text-sm"><IconClock size={18} /> Inativo</span>
          )}
        </td>
        ...
        <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center justify-end gap-1">
            {excluido ? (
              <button onClick={onRestaurar} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-semibold text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors">
                <IconRefresh size={18} /> Restaurar
              </button>
            ) : (
              <>
                <button onClick={onDetalhes} className="p-1.5 rounded-lg text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-[#2a2a2a] transition-colors" title="Detalhes">
                  <IconEye size={20} />
                </button>
                <button onClick={onEditar} className="p-1.5 rounded-lg text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors" title="Editar">
                  <IconEdit size={20} />
                </button>
                {!ehOProprioAdmin && u.permission !== "adm" && (
                  <button onClick={onExcluir} className="p-1.5 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Excluir">
                    <IconTrash size={20} />
                  </button>
                )}
              </>
        ...
        <td className="lg:hidden px-3 py-3 text-right">
          <span aria-hidden="true" className="inline-flex items-center justify-center p-1 rounded-md text-slate-400 dark:text-slate-500 pointer-events-none">
            {expandida ? <IconChevronUp size={15} /> : <IconChevronDown size={15} />}
          </span>
        </td>
      </tr>

      {/* Linha expandida — só abaixo de lg */}
      {expandida && (
          ...
          <td colSpan={3} className="px-4 py-3">
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
              ...
              <div className="sm:hidden">
                <dt className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">E-mail</dt>
                <dd className="text-sm text-slate-700 dark:text-slate-300 mt-0.5 break-all">{u.email}</dd>
              </div>

              <div className="md:hidden">
                <dt className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Matrícula</dt>
                <dd className="text-sm text-slate-700 dark:text-slate-300 mt-0.5 font-mono">{u.matricula}</dd>
              </div>

              <div>
                <dt className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Área</dt>
                <dd className="text-sm text-slate-700 dark:text-slate-300 mt-0.5">{u.area}</dd>
              </div>

              <div>
                <dt className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Perfil</dt>
                <dd className="mt-1"><BadgeUsuario label={PERMISSION_LABEL[u.permission]} cor={PERMISSION_COR[u.permission]} /></dd>
              </div>

              <div>
                <dt className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Status</dt>
                <dd className="mt-0.5">
                  {excluido ? (
                    <span className="inline-flex items-center gap-1 text-red-500 dark:text-red-400 font-semibold text-sm"><IconCircleX size={14} /> Excluído</span>
                  ) : u.ativo ? (
                    <span className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-semibold text-sm"><IconCircleCheck size={14} /> Ativo</span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-slate-400 dark:text-slate-500 text-sm"><IconClock size={14} /> Inativo</span>
                  )}
                </dd>
              </div>
