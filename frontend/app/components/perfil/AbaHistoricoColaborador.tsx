// app/components/perfil/AbaHistoricoColaborador.tsx
"use client";
...
export function AbaHistoricoColaborador({ matricula }: Props) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <SectionHeader
          icone={<IconCalendar size={16} className="text-amber-600 dark:text-[#FFDE00]" />}
          titulo="Eventos"
        ...
        {eventos.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-2 bg-white dark:bg-[#202020] rounded-2xl border border-slate-100 dark:border-[#303030]">
            <IconCalendarOff size={24} className="text-slate-300 dark:text-slate-600" />
          ...
          <div className="space-y-3">
            {eventos.map((evento) => {
              ...
              return (
                <div key={evento.id} className="bg-white dark:bg-[#202020] rounded-2xl border border-slate-100 dark:border-[#303030] shadow-sm overflow-hidden">
                  {/* Linha principal */}
                  <div className="flex items-start gap-4 p-4 flex-wrap sm:flex-nowrap">
                    ...
                    <div className="w-11 h-11 rounded-xl overflow-hidden shrink-0 bg-gradient-to-br from-[#FFDE00]/60 to-amber-400">
                      {evento.banner && (
                        <img src={evento.banner} alt="" className="w-full h-full object-cover" />
                      )}
                    </div>

                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-slate-900 dark:text-white text-sm line-clamp-1">{evento.nome}</p>
                      ...
                      {total > 0 && (
                        <div className="mt-2 w-full max-w-[200px]">
                          ...
                          <p className="text-[10px] text-slate-400 mt-0.5">{porcento}% de presença confirmada</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      {total > 0 && (
                        <button
                          onClick={() => setExpandidoEventoId(expandido ? null : evento.id)}
                          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-[#404040] text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-[#2a2a2a] transition-colors"
                        >
                          <IconUsers size={13} /> {expandido ? "Ocultar" : "Inscritos"}
                        </button>
                      )}
                    </div>
                  </div>
                        ...
                        <table className="w-full text-left text-xs">
                          <thead>
                            <tr className="border-b border-slate-100 dark:border-[#2a2a2a]">
                              <th className="px-4 py-2 font-bold text-slate-500 uppercase tracking-wide">Nome</th>
                              <th className="px-4 py-2 font-bold text-slate-500 uppercase tracking-wide hidden sm:table-cell">Matrícula</th>
                              <th className="px-4 py-2 font-bold text-slate-500 uppercase tracking-wide">Presença</th>
                            </tr>
                          </thead>
                          <tbody>
                            {inscricoes.map((insc) => (
                              <tr key={insc.id} className="border-b border-slate-100 dark:border-[#2a2a2a] last:border-0 hover:bg-slate-100 dark:hover:bg-[#252525]">
                                <td className="px-4 py-2 text-slate-700 dark:text-slate-300 font-medium">{insc.alunoNome}</td>
                                <td className="px-4 py-2 text-slate-500 font-mono hidden sm:table-cell">{insc.alunoMatricula}</td>
      ...
      <div>
        <SectionHeader
          icone={<IconSpeakerphone size={16} className="text-amber-600 dark:text-[#FFDE00]" />}
        ...
        {comunicados.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-2 bg-white dark:bg-[#202020] rounded-2xl border border-slate-100 dark:border-[#303030]">
            <IconSpeakerphone size={24} className="text-slate-300 dark:text-slate-600" />
          ...
          <div className="space-y-3">
            {comunicados.map((c) => {
              ...
              return (
                <div key={c.id} className="bg-white dark:bg-[#202020] rounded-2xl border border-slate-100 dark:border-[#303030] shadow-sm p-4 flex items-start gap-4 flex-wrap sm:flex-nowrap">
                  <div className="w-11 h-11 rounded-xl bg-[#FFDE00]/15 dark:bg-[#FFDE00]/10 flex items-center justify-center shrink-0">
                    <IconSpeakerphone size={18} className="text-amber-600 dark:text-[#FFDE00]" />
                  </div>

                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-slate-900 dark:text-white text-sm line-clamp-1">{c.titulo}</p>
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      {c.assunto && (
                        <span className="text-[10px] font-bold text-amber-500 dark:text-amber-400 uppercase tracking-wider">{c.assunto}</span>
                      )}
                      <span className="text-xs text-slate-400">{dataFormatada}</span>
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 line-clamp-1">{c.resumo}</p>
                  </div>
